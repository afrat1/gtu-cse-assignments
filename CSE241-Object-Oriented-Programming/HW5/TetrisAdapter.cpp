...
template <typename T>
TetrisAdapter<T>::TetrisAdapter(int row_i, int column_j) : row(row_i), column(column_j) {
...
template <typename T>
T& TetrisAdapter<T>::operator() (int x, int y) {
  return oneArray[y * column + x];
}

template <typename T>
void TetrisAdapter<T>::getArray(){
...
template <typename T>
void TetrisAdapter<T>::readsFromFile(){
...
void TetrisAdapter<T>::writeToFile(){
    ...
    for (int i = 0; i < row; i++) {
        for (int j = 0; j < column; j++) {
            output << gameOneArray[i*column+j] << " ";
        }
        output << std::endl;
    }
    output.close();
...
template <typename T>
void TetrisAdapter<T>::printArray(char* gameOneArray){
    cout << "\033[0;34m"<< "X" << "\033[0m";
    for(int a=0 ; a<column+1 ; a++) //cout << "\033[0;34m" << board[i][j] << "\033[0m";
        cout << "\033[0;34m"<< "X " << "\033[0m";
    cout << endl;
...
template <typename T>
void TetrisAdapter<T>::tetraminoAtTop(char* gameOneArray,char **tetramino){
...
template <typename T>
void TetrisAdapter<T>::rotatedVersionAndMove(char* gameOneArray,char **tetramino,char moveDirection, int moveCount){
...
template <typename T>
bool TetrisAdapter<T>::toCheckAnyTetramino(int *holder_value,char* gameOneArray,char moveDirection,int moveCount,char wordHolder){
    int i,j;
    bool flag = true;
    bool flag2 = true;
...
template <typename T>
int TetrisAdapter<T>::whichRow(int *holder_value,char* gameOneArray,char moveDirection,int moveCount,char wordHolder){
    int i,j;
    bool flag = true;
    bool flag2 = true;
    int rowss;
...
template <typename T>
void TetrisAdapter<T>::rotate_suitable(char **tetramino,char rotationDirection, int rotationCount){
